...
5. **Weather archive (training data, hourly):** the same `scheduled()` handler also calls `recordWeatherHour()` in **`weather-history.js`**, which fetches Open-Meteo and writes one row per trailhead per hour into `weather_hourly` in the same D1 database. Schema and reasoning: `migrations/0002_weather_history.sql`. This exists to answer one question — **when will a closed trail reopen?** — and nothing reads it yet.
   ...
   **`forecast_snapshots` is the only table here that cannot be backfilled.** `past_days` returns the model's after-the-fact *analysis*, never the forecast that was actually available at the time, so backtesting "what would we have predicted on Tuesday" against the analysis grades the model with hindsight it never had and every score comes out flattering. One vintage per trail per day is captured; the guard is simply "is the newest snapshot under a day old".
   ...
   Everything else here **is** backfillable, but **`past_days=92` does not mean 92 days of data.** Open-Meteo returns the full 92-day time axis and pads the far end with nulls: the real data starts **2026-07-04 08:00 UTC** for the HRRR variables and **2026-07-06 14:00 UTC** for the ICON soil ones. Everything before that is a row of NULLs — 38,686 of the 129,398 rows, about a third. Query `WHERE precip_in IS NOT NULL` (86,072 rows) when you want the usable record, and do not read `COUNT(*)` as coverage.
   ...
   **Some closures are scheduled, and they must not be fitted as drying events.** Big Cedar is closed **every Sunday morning (reopening about 1pm) and all day Monday** — a standing steward closure with nothing to do with weather, recorded from the steward on 2026-09-04. It is encoded as `scheduledClosure: { days: [0, 1] }` on that entry in `trails.js` (local weekday, 0 = Sunday), and `build-episodes.js` sets `scheduled: 1` on any episode the schedule fully explains. It **flags rather than filters** — excluding data is the modeller's call at fit time, and a mis-set schedule that silently deleted rows would be far harder to spot than one that mislabels them.

   Two details there are load-bearing. The weekday is **local (America/Chicago), never UTC**: the Monday closure begins around 9:40pm Sunday local, which is already Monday in UTC, so a UTC weekday gets it exactly backwards. And there is a **six-hour trailing grace**, because the steward reopens at 1:00am Tuesday rather than at midnight — without it that one hour of Tuesday rejects the whole episode. The grace is one-sided (the closure must still *start* on a scheduled day), so a genuine weather closure that happens to begin on a Sunday and run to Thursday is not written off.

   **Classify the closure before predicting anything — `closure-classify.js`.** "When will this trail open next?" is only a weather question for closures the weather caused. A standing schedule is a calendar lookup, a concert is unknowable, a trail shut since March is not drying at all. `classifyClosure()` returns one of `weather` / `scheduled` / `known-non-weather` / `stale` / `unexplained`, and **only `weather` is `predictable`**; everything else must decline to answer rather than emit a date it cannot justify.

   It deliberately uses **only what is knowable at closure time** — no reopen — because a live predictor will not have one, and a labeller that used it would train on information prediction never has. The same module is imported by `scripts/build-episodes.js`, so the training labels and any future live path cannot drift.

   **The status field cannot tell you the trail is wet.** 55 of 58 trailheads are Trailforks *regions*, which report a bare `Open`/`Closed` with no condition; only `trinity-track`, `western-heritage-park` and `erwin-park-skill-park` report the `Ideal`/`Dry`/`Wet`/`Prevalent Mud` vocabulary. For 95% of the network wetness has to be **inferred from `weather_hourly`** — which is the entire reason that table exists.

   **Measure drying from when the ground last got wet, not from `closed_at`.** `wetted_at` / `hours_drying` anchor to the last hour with rain, so a storm that continues after the gate shuts restarts the clock. Anchoring to `closed_at` assumes drying began the instant the trail closed, which is wrong in exactly the multi-day storm case where the answer matters most.

   **A scheduled closure does not start at midnight, and that cost us a false positive.** Big Cedar's *Sunday* closure was observed beginning **11:28pm Saturday** local. A bare weekday test called that Saturday, missed the schedule, and promoted a routine closure to the archive's only "usable drying event" — with zero rainfall behind it. `LEAD_GRACE_H` (6 hours) now leads the window as `REOPEN_GRACE` trails it; both are one-sided, so an ordinary weekday closure is still not swallowed.
